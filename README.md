...
*   **시각적 현실감 및 UI 개선**:
    *   트랙터 바퀴 조향 및 트레일러 '목' 부분의 시각적 현실감을 높였습니다.
    *   컨테이너 부분에 디테일을 추가하여 실제감 있는 외형을 구현했습니다.
    *   '트레일러 총 길이' UI를 직관적으로 개선했습니다.
*   **스크린샷**:    ![시뮬레이터 스크린샷](images/simulator_screenshot.png)


## 파일 구성

*   `truck_sim.py`: 메인 시뮬레이터 프로그램입니다.
...
`truck_sim.py`를 실행하여 시뮬레이터를 시작합니다.
...
*   **트레일러 총 길이**: 슬라이더를 조절하여 트레일러의 총 길이를 10.5m에서 14.0m 사이로 변경할 수 있습니다. (하늘색 부분 2.0m + 분홍색 본체 부분)
    *   **기본 값**: 11.5m 로 설정되어 있습니다. (분홍색 본체 부분 9.5m 기준)
    *   **참고**: 슬라이더는 트레일러의 '총 길이'를 조절하며, 실제 시뮬레이션에서는 총 길이에서 하늘색 부분(2.0m)을 제외한 분홍색 본체 부분의 길이가 변경됩니다.
    *   현실을 반영하여 정밀하게 설계된 모델이 아니기 때문에 적당하게 적응해서 사용하셔야 합니다. 
*   **기어 및 조향**:
...
*   **Preset 로드/세이브**:
    *   현재 차량의 상태를 Preset에 저장할 수 있습니다. 
    *   저장된 Preset의 내용을 불러와 저장된 시점부터 플레이를 이어할 수 있습니다.  
*   **Free Set**:
    *   Free Set 버튼을 누르면 회색의 고스트 차량이 보입니다. 
    *   마우스로 차량 중심을 잡고 X/Y 이동시킬 수 있습니다.  
    *   마우스로 트랙터 앞부분을 잡고, 차량 전체(트레일러 포함)를 회전시킬 수 있습니다. 
    *   마우스로 트레일러 뒷부분을 잡고, 트레일러만 회전시킬 수 있습니다. 
    *   세팅이 끝나면 Save 해서 적용합니다.  
*   **배경 이미지 조정**:
    *   "배경 이미지 로드" 버튼으로 이미지를 불러온 후, X/Y 오프셋과 스케일 슬라이더를 이용해 코스와 차량의 위치를 맞출 수 있습니다.
    *   **참고**: 트랙터와 트레일러의 시각적 요소들이 변경되었으므로, `course_image_making.py`로 생성된 이미지에 대한 기존의 X/Y 오프셋 및 스케일 값(`-27, -11, 105`)은 다시 확인하고 조정해야 할 수 있습니다.
